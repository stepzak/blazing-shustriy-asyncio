use std::{cell::RefCell, collections::{BinaryHeap, HashMap, VecDeque}, iter, sync::{Arc, mpsc::{self, Receiver, Sender}}, time::{Duration, Instant}};

use pyo3::{exceptions::PyRuntimeError, prelude::*, types::{PyList, PyTuple}};

use crate::{future::{PyFuture, RustFuture}, task::{RustTask, StepResult, TaskId}};
...
impl GatherState {
    fn new(total: usize) -> Self {
        GatherState { total, completed: 0, results: Some(iter::repeat_with(|| None as Option<PyObject>).take(total).collect()) }
    }
...
impl EventLoop {
    fn new(py: Python) -> PyResult<Self> {
        ...
        let future_t = module.getattr("PyFuture").ok().or_else(|| {
             PyModule::import(py, "blazing_shustriy_asyncio.rust_core").ok()?.getattr("PyFuture").ok()
        }).unwrap().unbind();
    ...
    fn create_task(&mut self, gen: &Bound<'_, PyAny>, py: Python) -> RustFuture {
        let id = self.next_id;
        self.next_id += 1;
        let task = RustTask::new(gen, id);
        let fut = RustFuture::new();
        
        self.tasks_hm.insert(id, TaskEntry { 
            task, 
            future: fut.clone(), 
            pending_val: None, 
            pending_err: None 
    ...
    fn schedule_task(&mut self, id: TaskId) {
        ...
        if let Some(err) = entry.pending_err.take() {
            let current_future = entry.future.clone();
            drop(entry);
            
            current_future.set_exception(err, py)?;
    ...
    fn handle_yield(&mut self, id: TaskId, val: Py<PyAny>, py: Python) -> PyResult<()> {
        ...
        if let Some(sleep_type) = &self.sleep_type {
            if val_binded.is_instance(sleep_type.bind(py))? {
                ...
                self.timers.push(Timer { when: when, task_id: id });
                return Ok(());
            }
        }
        ...
        if let Some(fut_val) = &self.future_type {
            if val_binded.is_instance(fut_val.bind(py)).unwrap_or(false) {
                if let Ok(py_future_ref) = val.extract::<PyRef<'_, PyFuture>>(py) {
                    ...
                    inner_future.add_callback(move |res: PyObject| {
                        tx_clone.send((my_id, Ok(res))).ok();
                    }, py);
                    let tx_err_callback = self.wake_tx.clone();
                    inner_future.add_err_callback( move |err: PyErr| {
        ...
        if val_binded.hasattr("send").unwrap_or(false) {
            ...
            child_future.add_callback( move |res| {
                tx_clone.send((my_id, Ok(res))).ok();
            }, py);
            let tx_err_callback = self.wake_tx.clone();
            child_future.add_err_callback(move |err| {
                tx_err_callback.send((my_id, Err(err))).ok();
            }, py);
    ...
    fn handle_gather(&mut self, id: TaskId, gather: &Bound<'_, PyAny>, py: Python) -> PyResult<()> {
        ...
        for (idx, coro) in coros.iter().enumerate() {
            ...
            child_fut.add_callback(move |res| {
                let maybe_list = {
                ...
                if let Some(list) = maybe_list {
                    Python::with_gil(|py| {
                        ...
                        tx_cb.send((my_id, Ok(py_list.unwrap().into()))).ok();
                    });
                }
            }, py);
            let err_cb = tx_clone.clone();
            child_fut.add_err_callback(move |err| {
    ...
    fn run_forever(&mut self, py: Python) -> PyResult<()> {
        loop {
            ...
            if self.tasks_hm.is_empty() { break; }
            py.check_signals()?;
        }
        Ok(())
    }
...
pub struct RustEventLoop { event_loop: Arc<RefCell<EventLoop>> }
impl RustEventLoop {
    pub fn new(py: Python) -> PyResult<Self> { Ok(Self { event_loop: Arc::new(RefCell::new(EventLoop::new(py)?)) }) }
    pub fn create_task(&self, gen: &Bound<'_, PyAny>, py: Python) -> RustFuture { self.event_loop.borrow_mut().create_task(gen, py) }
    pub fn run_forever(&self, py: Python) -> PyResult<()> { self.event_loop.borrow_mut().run_forever(py) }
...
#[pyclass(unsendable)]
pub struct PyEventLoop { loop_impl: RustEventLoop }
#[pymethods]
impl PyEventLoop {
    #[new] fn new(py: Python) -> PyResult<Self> { Ok(Self { loop_impl: RustEventLoop::new(py)? }) }
    ...
    fn run_forever(&self, py: Python) -> PyResult<()> { self.loop_impl.run_forever(py) }
