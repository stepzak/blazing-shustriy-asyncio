...
pub fn json(
    ...
    let body_bytes = if let Ok(py_bytes) = data.cast::<pyo3::types::PyBytes>() {
    ...
    } else if data.is_instance_of::<pyo3::types::PyDict>() || data.is_instance_of::<pyo3::types::PyList>() {
        ...
        let bytes_obj: Bound<'_, pyo3::types::PyBytes> = if orjson.name()? == "orjson" {
            ...
            orjson.call_method1("dumps", (data,))?
                  .call_method0("encode")?
                  .cast_into()?
        };
    ...
    if !res_headers.iter().any(|(k, _)| k.eq_ignore_ascii_case("Content-Type")) {
